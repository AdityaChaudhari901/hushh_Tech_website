/**
 * Wallet pass model — CommonJS-compatible JS for Vercel Node API routes.
 * Keep in sync with src/shared/walletPassModel.ts (the authoritative source
 * used by the Vite frontend build).
 */
...
export const getWalletPayloadFieldValue = (fields, key, fallback = "") => {
  ...
  const field = fields.find(
    (item) => item !== null && typeof item === "object" && item.key === key
  );
...
const buildInvestmentLabel = (investmentClass) =>
